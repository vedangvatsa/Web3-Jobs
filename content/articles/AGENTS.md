...
- Credit the hero image with `imageCaption` (short description plus photographer and license) and `imageCreditUrl` (link to the source page) in front matter. The route renders the caption centered directly below the hero image. Do not repeat the same credit as a second inline image in the body.
...
- On GitHub Actions, do not launch the site and do not signal other processes. Check the files, then commit. Opening a page is only for a machine where the site is already running.
