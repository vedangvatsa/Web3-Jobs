...
Operate on core overlap for decisions and async for everything else. Rotate meeting pain across zones instead of taxing one region forever. Record everything by default.
...
Publish the framework before hiring. Secret pay bands breed resentment that public treasuries make visible anyway.
...
Default to stablecoin base with token upside, invoiced monthly, reconciled on-chain. Keep one system of record both accountants and contributors can read.
...
The pattern across all three: written values, high bars, async defaults, and pay that competes globally instead of locally.
