...
**Option B: ZKSync Era with zksync2-swift** Follow the same Swift Package step, then use `ZkSyncClient` at `https://sepolia.era.zksync.dev` and `WalletL2` for `getBalance` and `transfer`. Keep the same web3swift ABI for contracts.
