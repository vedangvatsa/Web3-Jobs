...
**Why is `k` constant?**`k` is the invariant the AMM is designed to maintain. It changes only when liquidity providers add or remove assets from the pool, which establishes a new `k`.
