...
- **Sandwich Attack:** This is a combination of front-running and back-running. The bot front-runs the user's buy order, lets the user's transaction execute (pushing the price up further), and then immediately sells its own tokens. The user's trade is "sandwiched" between the bot's buy and sell, resulting in a worse price for the user and a profit for the bot.
