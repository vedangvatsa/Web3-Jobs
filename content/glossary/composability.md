...
**Composability** is DeFi's superpower—ability to combine protocols like building blocks. Uniswap swap output automatically flows to lending protocol. Lending protocol collateral automatically provides liquidity to other protocols. Complex strategies from simple building blocks. Composability enables innovation: yield farming combines staking + lending + DEX trading. Composability enables wealth creation: DeFi Summer 2020 explosive growth due to composability. But composability also creates risks: if one protocol hacked, affects all dependent protocols (cascade failures). Composability is powerful but creates interconnectedness risks.
