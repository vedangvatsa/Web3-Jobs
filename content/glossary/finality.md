...
**Finality** is when transaction becomes permanently irreversible. Bitcoin: After 6 confirmations (~1 hour), transaction considered final. 51% attack possible but prohibitively expensive. Ethereum PoS: After ~13 minutes, transaction has economic finality (validator penalties prevent reversal). Shorter finality = faster transactions but more reversal risk. Longer finality = more secure but slower. Different chains have different finality periods. Finality is critical for high-value transactions requiring certainty. Understanding finality is essential for evaluating blockchain security.
