...
req2 = urllib.request.Request('https://cloudbuild.googleapis.com/v1/projects/web3-jobs-aggregator/builds?pageSize=10', headers={'Authorization': f'Bearer {token}'})
builds_res = json.loads(urllib.request.urlopen(req2).read())

builds = builds_res.get('builds', [])
latest_id = builds[0]['id'] if builds else None

print(f"Latest Build: {latest_id}")
for b in builds:
    b_id = b['id']
    status = b['status']
    if b_id != latest_id and status in ['WORKING', 'QUEUED']:
        print(f"Cancelling stale build {b_id}...")
        cancel_req = urllib.request.Request(f'https://cloudbuild.googleapis.com/v1/projects/web3-jobs-aggregator/builds/{b_id}:cancel', data=b'{}', headers={'Authorization': f'Bearer {token}', 'Content-Type': 'application/json'})
        try:
            urllib.request.urlopen(cancel_req)
            print(f"Cancelled {b_id}")
        except Exception as e:
            print(f"Failed to cancel {b_id}: {e}")
