...
req2 = urllib.request.Request('https://cloudbuild.googleapis.com/v1/projects/web3-jobs-aggregator/builds?pageSize=3', headers={'Authorization': f'Bearer {token}'})
...
for b in builds_res.get('builds', []):
    ...
    finish = b.get('finishTime', 'IN_PROGRESS')
    print(f"Build ID: {build_id} | Status: {status} | Created: {created} | Finished: {finish}")
