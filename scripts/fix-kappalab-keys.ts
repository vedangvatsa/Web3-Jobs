...
function fix() {
  ...
  if (fs.existsSync(descPath)) {
    cache = JSON.parse(fs.readFileSync(descPath, 'utf8'));
  }

  const quantKey = getJobContentKey(quantTraderJob);
  const bdKey = getJobContentKey(bdManagerJob);

  cache[quantKey] = quantTraderHtml;
  cache['kappa-lab-quant-trader-297472'] = quantTraderHtml;
  cache['trader'] = quantTraderHtml;

  cache[bdKey] = bdManagerHtml;
  cache['kappa-lab-bd-manager-292292'] = bdManagerHtml;
  cache['bd'] = bdManagerHtml;

  fs.writeFileSync(descPath, JSON.stringify(cache, null, 2));
  console.log(`Injected Quant Trader content into key: ${quantKey}`);
  console.log(`Injected BD Manager content into key: ${bdKey}`);
}
