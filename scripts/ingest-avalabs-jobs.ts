...
async function fetchAvaLabsJobs() {
  console.log('Fetching live Ashby board for ava-labs...');
  const res = await fetch('https://jobs.ashbyhq.com/ava-labs');
  ...
  console.log(`Found ${postings.length} active job postings for Ava Labs.`);
  ...
  for (const posting of postings) {
    ...
    const link = `https://jobs.ashbyhq.com/ava-labs/${jobId}`;
    
    // Fetch full HTML description via Ashby GraphQL API
    let descriptionHtml = '';
    try {
      const gqlRes = await fetch('https://jobs.ashbyhq.com/api/non-user-graphql?op=ApiJobPosting', {
        ...
        body: JSON.stringify({
          ...
          variables: { organizationHostedJobsPageName: 'ava-labs', jobPostingId: jobId }
        })
      });
    ...
    const jobRecord = {
      ...
      link,
      date: nowIso,
      source: 'Ashby: Ava Labs [ava-labs]',
      location: loc,
  ...
  console.log(`Successfully ingested Ava Labs jobs: ${added} added, ${updated} updated. Total jobs in cache: ${jobs.length}`);
}

fetchAvaLabsJobs().catch(console.error);
