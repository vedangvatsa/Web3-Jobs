...
const SCRIPT_PATH = path.join(process.cwd(), 'scripts/fetch-coindcx-jobs.py');

export async function ingestCoinDCXOfficial() {
  console.log('--- Ingesting Official CoinDCX Job Openings ---');
  try {
    const pyCode = `
...
`;
    fs.writeFileSync(SCRIPT_PATH, pyCode.trim());
    execFileSync('python3', [SCRIPT_PATH], { stdio: 'inherit' });
  } catch (err: any) {
    console.error(`✗ [CoinDCX Official Portal] Ingestion failed: ${err.message}`);
  }
}

if (require.main === module) {
  ingestCoinDCXOfficial();
}
