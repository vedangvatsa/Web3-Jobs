...
function ingestSkyMavis() {
  const rawJobs = JSON.parse(fs.readFileSync('skymavis-raw.json', 'utf8'));
  ...
  for (const j of rawJobs) {
    ...
    if (!cacheData.some((existing: any) => existing.id === job.id)) {
      cacheData.unshift(job);
      added++;
    }
  }

  console.log(`Ingested ${added} fresh (<= 30 days old) jobs for Sky Mavis. Total jobs in cache: ${cacheData.length}`);
