...
const descriptionStore = readJobDescriptionStore();
const jobDescriptions = descriptionStore.descriptions;
...
(async () => {
  ...
  for (const wd of workdayBoards) {
    ...
    for (const p of data.jobPostings) {
      ...
      if (description) jobDescriptions[getJobContentKey(jobObj)] = description;
      totalAdded++;
    }
  }
  ...
  writeJobDescriptionStore({
    ...
    aliases: {
      ...descriptionStore.aliases,
      ...buildJobDescriptionAliases(jobsCache, jobDescriptions),
    },
  });
