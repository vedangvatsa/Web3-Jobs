...
req2 = urllib.request.Request('https://cloudbuild.googleapis.com/v1/projects/web3-jobs-aggregator/builds?pageSize=1', headers={'Authorization': f'Bearer {token}'})
builds_res = json.loads(urllib.request.urlopen(req2).read())

b = builds_res.get('builds', [])[0]
build_id = b.get('id')
status = b.get('status')
print(f"Build ID: {build_id} | Status: {status} | Created: {b.get('createTime')}")
steps = b.get('steps', [])
for i, s in enumerate(steps):
    print(f"  Step {i} ({s.get('name')}): {s.get('status')}")
