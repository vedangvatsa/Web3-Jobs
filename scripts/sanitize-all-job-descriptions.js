...
function sanitizeHtml(html) {
  ...
  cleaned = cleaned.replace(/\s*style="min-height:\s*1\.5em;?"/gi, "");
  cleaned = cleaned.replace(/\s*style="min-height:\s*1\.7em;?"/gi, "");

  // Fix corrupted href attributes (e.g. href="" http:="" www.domain.com... -> href="https://www.domain.com")
  ...
  // 5. Strip unneeded span tags wrapping plain text with no meaningful attributes
  ...
  // 6. Remove tracking code tags (e.g., #LI-DNP)
  ...
  // 8. Fix mid-sentence line breaks (<br> followed by lowercase letter or common punctuation)
  ...
  cleaned = cleaned.replace(/<p[^>]*>\s*([*•\-]|&bull;)\s+(.*?)(?:<br\s*\/?>|\n|\r)+(.*?)\s*<\/p>/gi, (match) => {
    const items = match.replace(/<\/?p[^>]*>/g, '').split(/<br\s*\/?>|\n|\r/).map(line => {
      const trimmed = line.replace(/^\s*[*•\-]|&bull;\s*/, '').trim();
      return trimmed ? `<li>${trimmed}</li>` : '';
    }).filter(Boolean);
    return `<ul>${items.join('')}</ul>`;
  });

  // Convert lone <p>* bullet item</p> to <li> wrapped in <ul>
