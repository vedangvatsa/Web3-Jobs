...
const TARGETS_BATCH_2: TargetEvent[] = [
  {
    eventName: "Taipei Blockchain Week",
    to: "partners@taipeiblockchainweek.com",
    cc: ["info@taipeiblockchainweek.com", "contact@taipeiblockchainweek.com"]
  },
  {
    eventName: "Binance Blockchain Week",
    to: "events@binance.com",
    cc: ["ella.m@binance.com"]
  },
  {
    eventName: "UN Blockchain Week",
    to: "contact@blockchainweek.ai",
    cc: ["info@blockchainweek.ai"]
  },
  {
    eventName: "Paris Blockchain Week",
    to: "contact@chainof.events",
    cc: ["support@parisblockchainweek.com", "contact@blockchainweekparis.com"]
  },
  {
    eventName: "Istanbul Blockchain Week",
    to: "ibw@eakdigital.com",
    cc: ["partnerships@eakdigital.com", "contact@eakdigital.com"]
  },
  {
    eventName: "ETHSafari",
    to: "partners@ethsafari.xyz",
    cc: ["hello@ethsafari.xyz", "sponsor@ethsafari.xyz"]
  },
  {
    eventName: "ETHCC",
    to: "contact@ethereum-france.com",
    cc: ["sponsors@ethcc.io"]
  },
  {
    eventName: "ETHDam",
    to: "hello@cryptocanal.org",
    cc: ["sponsor@ethdam.com"]
  },
  {
    eventName: "ETHPrague",
    to: "contact@ethprague.com",
    cc: ["gm@ethprague.com"]
  },
  {
    eventName: "ETHBerlin",
    to: "contact@ethberlin.org",
    cc: ["sponsor@ethberlin.org"]
  },
  {
    eventName: "Ethereum Korea One",
    to: "contact@ethereumkorea.io",
    cc: ["one@ethereumkorea.io"]
  },
  {
    eventName: "Ethereum Cypherpunk Congress",
    to: "web3privacynow@protonmail.com",
    cc: ["congress@web3privacy.info"]
  },
  {
    eventName: "ETHWomen & Blockchain Futurist Conference",
    to: "info@futuristconference.com",
    cc: ["events@untraceableinc.com"]
  }
];
...
async function run() {
  ...
  console.log("Starting dispatch to " + TARGETS_BATCH_2.length + " additional major events...\n");

  const results: any[] = [];

  for (let i = 0; i < TARGETS_BATCH_2.length; i++) {
    const t = TARGETS_BATCH_2[i];
    console.log(`[${i + 1}/${TARGETS_BATCH_2.length}] Sending to ${t.eventName} (${t.to})...`);
    ...
    if (i < TARGETS_BATCH_2.length - 1) {
      await sleep(1200);
    }
  }

  console.log("\n========================================");
  console.log("Dispatch Summary Batch 2:");
  const successful = results.filter(r => r.success).length;
  console.log(`Successfully sent: ${successful}/${TARGETS_BATCH_2.length}`);
  console.log("========================================");
}
