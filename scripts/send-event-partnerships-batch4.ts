...
const TARGETS_BATCH_4: TargetEvent[] = [
  {
    eventName: "Japan Blockchain Week",
    to: "contact@japanblockchainweek.jp",
    cc: ["info@japanblockchainweek.jp"]
  },
  {
    eventName: "Australian Blockchain Week",
    to: "info@blockchainaustralia.com.au",
    cc: ["events@blockchainaustralia.com.au"]
  },
  {
    eventName: "ETHWarsaw",
    to: "hello@ethwarsaw.dev",
    cc: ["sponsors@ethwarsaw.dev"]
  },
  {
    eventName: "ETHBelgrade",
    to: "contact@ethbelgrade.rs",
    cc: ["sponsor@ethbelgrade.rs"]
  },
  {
    eventName: "ETHLisbon",
    to: "contact@ethlisbon.org",
    cc: ["sponsor@ethlisbon.org"]
  },
  {
    eventName: "ETHSeoul",
    to: "contact@ethseoul.org",
    cc: ["sponsor@ethseoul.org"]
  },
  {
    eventName: "ETHMilan",
    to: "ciao@ethmilan.xyz",
    cc: ["contact@ethmilan.xyz"]
  },
  {
    eventName: "ETHBucharest",
    to: "contact@ethbucharest.xyz",
    cc: ["sponsors@ethbucharest.xyz"]
  },
  {
    eventName: "ETHKL",
    to: "contact@ethkl.org",
    cc: ["hello@ethkl.org"]
  }
];
...
async function main() {
  ...
  console.log(`Starting dispatch to 9 additional major events (Batch 4)...`);

  let successCount = 0;
  let failCount = 0;

  for (let i = 0; i < TARGETS_BATCH_4.length; i++) {
    const target = TARGETS_BATCH_4[i];
    const subject = `Official Community Partnership Proposal // ${target.eventName} x Hashtag Web3`;

    console.log(`\n[${i + 1}/${TARGETS_BATCH_4.length}] Sending to ${target.eventName} (${target.to})...`);
    ...
    if (i < TARGETS_BATCH_4.length - 1) {
      await sleep(1500); // Respect rate limit
    }
  }

  console.log(`\n========================================`);
  console.log(`Dispatch Summary Batch 4:`);
  console.log(`Successfully sent: ${successCount}/${TARGETS_BATCH_4.length}`);
  if (failCount > 0) {
