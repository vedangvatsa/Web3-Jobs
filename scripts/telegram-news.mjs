...
async function filterAndSummarize(newsItems, recentHeadlines = [], pickCount = STORIES_PER_POST) {
  ...
  const prompt = `Pick the ${want} most important Web3 industry stories from these ${Math.min(newsItems.length, 30)} items.
...
Return ONLY a JSON array of exactly ${want} objects: {"index", "headline", "summary", "event_rationale"}
...
async function sendToTelegram(message) {
  ...
  const res = await fetch(url, {
    ...
    body: JSON.stringify({
      ...
      reply_markup: {
        inline_keyboard: [
          [{ text: 'Turn your CV into a Website', url: 'https://cvin.bio/?utm_source=social&utm_medium=telegram&utm_campaign=web3newsfeed' }],
        ],
      },
    }),
  });
...
async function postOnce() {
  ...
  const fresh = allNews.filter(n => {
    ...
    // Title-only: snippets share generic crypto wording and caused false blocks.
  ...
  if (stories.length < targetCount) {
    ...
    for (let i = 0; i < fresh.length; i++) {
      ...
      try {
        const summaryPrompt = `Rewrite this Web3 news headline and snippet into a factual plain-English digest.
Headline: ${item.title}
